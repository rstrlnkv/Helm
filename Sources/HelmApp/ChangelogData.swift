    ...
    static var entries: [ChangelogEntry] {
        [
            ChangelogEntry(version: "0.11.1", date: "2026-08-26", items: [
                ...
                ChangeItem(kind: .new, text: L("Homebrew’s search puts the packages most people install at the top of each group.")),
                ChangeItem(kind: .new, text: L("Uninstalling a package first says which of your installed packages still need it.")),
                ChangeItem(kind: .upd, text: L("The installed list has a heading and a version column, and every name starts at one left edge.")),
                ChangeItem(kind: .upd, text: L("“Upgrade all” sits in the page’s own bar, in a slot every segment keeps.")),
                ChangeItem(kind: .upd, text: L("At a narrow window Homebrew is two screens instead of one row with a button.")),
                ChangeItem(kind: .fix, text: L("Helm tells a brew question that came back empty from one that was refused and one that never ran — a refused package list used to spin for ever.")),
                ChangeItem(kind: .fix, text: L("The size of a package says it is being measured while the walk is out, instead of appearing out of nothing.")),
                ChangeItem(kind: .upd, text: L("Each settings page’s name and its switcher moved into the window’s toolbar.")),
                ChangeItem(kind: .new, text: L("Right-click the toolbar’s switcher to draw its segments as words, as glyphs, or as both.")),
                ChangeItem(kind: .upd, text: L("At the smallest window Refresh keeps its place in the toolbar, and a page’s name keeps its distance from the window’s edge.")),
                ChangeItem(kind: .upd, text: L("The menu-bar panel’s tabs, arranging bar and footer are drawn flat on the panel’s own glass, instead of glass over glass.")),
                ChangeItem(kind: .fix, text: L("The panel grows down to the Dock before it scrolls, so a tall screen holds the whole grid.")),
                ChangeItem(kind: .upd, text: L("The panel’s list fades where it continues past the edge.")),
                ChangeItem(kind: .new, text: L("Make a new SSH key from the + in the toolbar, on the Keys tab.")),
                ChangeItem(kind: .fix, text: L("Leftovers gives back the login items it switched off when you turn the module off — switching one off used to outlive deleting Helm.")),
                ChangeItem(kind: .fix, text: L("Helm refuses an update whose bundle calls itself something other than Helm.")),
            ...
            ChangelogEntry(version: "0.10.0", date: "2026-08-09", items: [
                ...
                ChangeItem(kind: .new, text: L("A colour of your own: the colour menu ends with \u{201C}Other colour\u{2026}\u{201D}, which opens the system colour panel \u{2014} the way Calendar offers one. The palette is Calendar\u{2019}s, and follows light and dark the way the system\u{2019}s colours do.")),
            ...
            ChangelogEntry(version: "0.8.0", date: "2026-07-29", items: [
                ...
                ChangeItem(kind: .new, text: L("Duplicates can mark every extra copy at once, and Space shows you a file before you bin it. One press does across every group what the per-group button did \u{2014} one copy of everything always stays, which is why the extras are marked rather than everything selected. Beside \u{201C}Move to Trash\u{201D} there is now \u{201C}Clear the marks\u{201D}, so one press can undo one press.")),
