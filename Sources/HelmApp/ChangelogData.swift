    ...
    static var entries: [ChangelogEntry] {
        [
            ...
            ChangelogEntry(version: "0.8.0", date: "2026-07-29", items: [
                ...
                ChangeItem(kind: .fix, text: L("The duplicate finder no longer holds on to everything it reads, so the memory a scan uses follows the size of a slice rather than the size of the folder. Memory is also handed back to macOS when a scan finishes and when a module is switched off.")),
                ...
                ChangeItem(kind: .upd, text: L("The Uninstaller\u{2019}s last screen before deleting now names the app itself, not only its leftover files. Homebrew says plainly that its removal skips the Trash and can\u{2019}t be undone \u{2014} it\u{2019}s the one deletion in Helm that really is permanent. Duplicates now confirms with the same count, size and named paths as Disk, and Disk\u{2019}s advice about an old file now cites the date it was last written rather than calling it \u{201C}untouched\u{201D}. Safari is marked as a system app rather than offered for removal at \u{201C}0 B\u{201D}.")),
                ...
                ChangeItem(kind: .upd, text: L("Warning and success colours, and the numbers in a metric strip, are legible in light appearance now, and the metric strip no longer occasionally reads the wrong appearance and darkens dark mode\u{2019}s own colours to match.")),
                ChangeItem(kind: .fix, text: L("The VPN dial counted connections that had started themselves automatically, sitting right above the list of rules it looked like it was counting. Login Items & Extensions\u{2019} bottom bar mixed how many items were found with the size of what you\u{2019}d selected.")),
            ]),
