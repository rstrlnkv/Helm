    ...
    static var entries: [ChangelogEntry] {
        [
            ChangelogEntry(version: "0.10.0", date: "2026-08-09", items: [
                ChangeItem(kind: .new, text: L("Autopilot offers five rules you can have without writing one \u{2014} screenshots into their own folder, downloads sorted by kind, old installers to the Trash, a tag on big downloads, the desktop sorted by month. A row opens the ordinary editor on a draft that is saved nowhere, so you see what it would do to your real folder first: cancel and nothing is left behind, confirm and the folder is tidied once straight away, with a way to put it all back.")),
                ...
                ChangeItem(kind: .fix, text: L("The battery guard says so on screen, names the setting that stopped it and how to lift it, and now reaches you with a notification even when you are not looking at Helm. It used to end sessions in silence \u{2014} you pressed \u{201C}15 min\u{201D} at 5% and nothing happened, with the reason only in the log.")),
                ...
                ChangeItem(kind: .new, text: L("The lid row says when sleep is off for the whole Mac right now, and how to bring it back \u{2014} it used to show only what turning the switch on costs, never what had happened once you had.")),
                ChangeItem(kind: .fix, text: L("A refused lid says so, instead of leaving the switch on with nothing behind it. If macOS declines to turn system sleep off, the row now says the Mac will sleep with the lid closed after all.")),
                ChangeItem(kind: .fix, text: L("Helm says when it cannot read your saved app rules, and to add them again. Unreadable rules used to stop holding the Mac awake in silence, with only one line in the diagnostics log \u{2014} the page looked exactly as if you had chosen no apps.")),
                ...
                ChangeItem(kind: .fix, text: L("A VPN rule that cannot get at its saved secret now says so on the page and asks for one press of Connect. It used to stop working in silence, with the reason only in the diagnostics log.")),
                ...
                ChangeItem(kind: .new, text: L("Autopilot can put back what it moved, renamed, tagged or deleted \u{2014} one file at a time from its own row, or a whole run at once from the report or the history below it, grouped by the sweep or the batch that made it. If a returned file\u{2019}s rule could take it again within the hour, the report offers to turn that rule off.")),
                ChangeItem(kind: .fix, text: L("Pressing \u{201C}Put back\u{201D} could crash Helm outright. It no longer can.")),
                ChangeItem(kind: .fix, text: L("A sorting rule stopped trying to file its own sorted folders inside themselves \u{2014} it used to fail at that every hour, quietly, forever.")),
                ChangeItem(kind: .fix, text: L("Duplicates could fail to find anything at all, silently. Search is repaired, along with a removal banner that overcounted the space you would free when Finder had its own clone of a file, and a removal that got no answer, which used to say nothing on a second press either.")),
                ChangeItem(kind: .fix, text: L("A copy you chose to keep by hand now stays chosen after a removal, instead of being un-chosen the moment that removal empties the basket it used.")),
                ChangeItem(kind: .fix, text: L("Disk says when a folder is still being measured, and Stop now really stops it. An unanswered request for your volumes no longer looks like a Mac with no disks on it. A file already gone by the time Disk tries to remove it gets its own message, instead of being sent to look for it in the Finder. And the free-space tile on the panel updates again, instead of freezing at whatever it read when Helm launched.")),
                ChangeItem(kind: .fix, text: L("The Log page now shows the log that is actually on disk, not only this session\u{2019}s \u{2014} yesterday\u{2019}s crash used to be missing from the very page you would open to find it.")),
                ChangeItem(kind: .fix, text: L("Removal, sweep and \u{201C}Put back\u{201D} reports are now announced to VoiceOver as they appear, and the welcome tour moves VoiceOver\u{2019}s focus along with each step instead of leaving it on the Next button.")),
                ChangeItem(kind: .new, text: L("Removing duplicates shows its progress and can be stopped. “Stop removal” ends it where it is — what already moved stays in the Trash, and the report says the removal was stopped instead of calling it a failure.")),
                ChangeItem(kind: .new, text: L("A Homebrew operation can be stopped, and stopping it is reported as “Stopped”, not as a failure. A brew that stops answering no longer hangs the module — Helm cuts it off after a minute and a half and keeps the last good answer. And if Helm quits while brew is still working, the next launch says so once in the console.")),
                ChangeItem(kind: .new, text: L("Keyboard speaks with VoiceOver: a conversion is announced with its words, the pause at a password field and a revoked permission are announced as they happen, and the shortcut recorder reads its combination back in words. The note under the tap key warns that tapping Control on its own is also VoiceOver’s pause-speech gesture.")),
                ChangeItem(kind: .fix, text: L("An Autopilot rule with a half-written condition can no longer be switched on. “Name begins with” and nothing typed matched every file — with the action set to Trash, three gestures from a blank rule were enough to endanger a whole folder. The rule is kept, switched off, for you to finish.")),
                ChangeItem(kind: .fix, text: L("“New rule” is a draft until you press Done. It used to be saved the moment the editor opened, so Cancel left an Untitled rule behind.")),
                ChangeItem(kind: .upd, text: L("Autopilot’s history keeps up while you watch: a sweep or a file handled in the background now appears on the open page, instead of waiting for you to reopen it.")),
                ChangeItem(kind: .upd, text: L("A folder Autopilot may never watch — a system folder, a whole disk — is refused in its own words now, instead of sending you to grant Full Disk Access that would not have helped.")),
                ChangeItem(kind: .upd, text: L("Autopilot reads big folders noticeably faster: it no longer weighs what no rule asks about, and the editor no longer runs its preview twice.")),
                ...
                ChangeItem(kind: .fix, text: L("A word fixed by selecting it now counts into the day’s figure.")),
                ChangeItem(kind: .upd, text: L("A change you rejected stays on the page with “Never this word” beside it, and the words on that list now win over the forced gesture too.")),
                ChangeItem(kind: .fix, text: L("Keyboard says “Paused” while a password field is actually in front, not at the next conversion — and when all three of its switches are off, the page says so instead of looking configured.")),
                ChangeItem(kind: .fix, text: L("Homebrew no longer claims “Updates: 0” before it has asked — the line stays quiet about updates until the answer arrives.")),
                ChangeItem(kind: .fix, text: L("A pair of duplicates Helm cannot read gets its own explanation, instead of “this is not where Helm found it” about a file that is exactly there — and every refusal on the page now ends by offering “Search again”, the button the page actually has.")),
                ChangeItem(kind: .fix, text: L("A new duplicates search no longer shows the last one’s report, and a stopped search says it was stopped instead of pretending no folder was chosen.")),
                ChangeItem(kind: .upd, text: L("A duplicate group’s header now says what removing its extras would actually free, instead of the size times the count — which promised too much whenever the copies were clones sharing their space. The honest total is drawn at every window width now.")),
                ChangeItem(kind: .fix, text: L("Helm’s memory no longer grows while its windows are closed. A page in a closed window kept rendering — and keeping — every update; it is put away now and rebuilt in a blink when the window comes back. A VPN check that learned nothing new also stopped redrawing every page all night.")),
                ChangeItem(kind: .fix, text: L("Big counts read with digit grouping in your language — “1,499,308 files”, never “1499308 files” — everywhere a count is drawn.")),
                ChangeItem(kind: .upd, text: L("The log’s filter row keeps to one line in every language: Follow is a glyph now, with its word in the tooltip.")),
