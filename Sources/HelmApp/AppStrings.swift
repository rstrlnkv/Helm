...
enum AppStr {
    ...
    static var channelBeta: String { L("Beta", [.ru: "Бета", .es: "Beta", .fr: "Bêta", .de: "Beta", .ja: "ベータ", .zh: "测试版", .pt: "Beta"]) }
    static var channelDev: String { L("Dev", [.ru: "Dev", .es: "Dev", .fr: "Dev", .de: "Dev", .ja: "Dev", .zh: "开发版", .pt: "Dev"]) }
    static var channelBetaNote: String { L("Finished releases only. Helm is still before 1.0, so they are beta too — just tested first.", [.ru: "Только законченные релизы. Helm ещё не дошёл до 1.0, так что и они бета — просто проверенная.", .es: "Solo versiones terminadas. Helm aún no llega a 1.0, así que también son beta: solo que probadas antes.", .fr: "Uniquement les versions finalisées. Helm n’a pas atteint la 1.0 : elles restent bêta, mais testées d’abord.", .de: "Nur fertige Releases. Helm ist noch vor 1.0, also sind auch sie Beta — nur zuerst getestet.", .ja: "完成版リリースのみ。Helm はまだ 1.0 前なので、これらもベータです（先にテスト済み）。", .zh: "仅接收完成的版本。Helm 尚未到 1.0，所以它们同样是测试版——只是先经过验证。", .pt: "Apenas versões finalizadas. O Helm ainda não chegou à 1.0, então elas também são beta — só que testadas antes."]) }
    static var betaBadge: String { L("BETA", [.ru: "БЕТА", .es: "BETA", .fr: "BÊTA", .de: "BETA", .ja: "ベータ", .zh: "测试版", .pt: "BETA"]) }
