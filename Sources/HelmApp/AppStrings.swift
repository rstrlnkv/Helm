...
enum AppStr {
    ...
    static var channelBetaNote: String { L("Helm is still in development. Beta builds are the steadier of the two.", [.ru: "Helm ещё в разработке. Beta-сборки — более устойчивые из двух.", .es: "Helm sigue en desarrollo. Las builds Beta son las más estables de las dos.", .fr: "Helm est encore en développement. Les versions Beta sont les plus stables des deux.", .de: "Helm ist noch in Entwicklung. Beta-Builds sind die stabileren von beiden.", .ja: "Helm はまだ開発中です。2 つのうち Beta のほうが安定しています。", .zh: "Helm 仍在开发中。两者之中 Beta 更稳定。", .pt: "O Helm ainda está em desenvolvimento. As builds Beta são as mais estáveis das duas."]) }
