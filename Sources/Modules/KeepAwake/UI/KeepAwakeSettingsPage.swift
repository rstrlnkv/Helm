...
public struct KeepAwakeSettingsPage: View {
    ...
    /// `onTapGesture` never enters the key-view loop, so with Full Keyboard
    /// Access on the Tab key skipped every swatch and the colour could not be
    /// chosen without a mouse — VoiceOver worked, which is what hid it. Ten
    /// `KeyViewProxy` views appear under the hosting view for the button form
    /// and none for the gesture form; `.buttonStyle(.plain)` leaves the drawing
    /// exactly as it was.
