...
/// The search root, as the diagnostics log may carry it.
///
...
/// work and wrong for a search root, because a search root is chosen by the
/// person using it: `/Volumes/Anna's Work Backup` names an employer,
/// `/Users/anna` names an account. Both then sat in the file whose purpose is
/// being attached to a bug report.
///
...
/// app.
///
/// The twin of this lives in the Disk module, which logs a root of its own. The
/// rule belongs beside `Redact` in HelmRuntime and should move there the next
/// time that file is open; it is spelled twice rather than reached for across
/// two modules that share nothing else.
enum LogRoot {
    ...
    static func label(_ root: String, home: String = NSHomeDirectory()) -> String {
