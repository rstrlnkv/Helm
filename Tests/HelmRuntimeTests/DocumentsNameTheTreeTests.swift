...
final class DocumentsNameTheTreeTests: XCTestCase {
    ...
    private static let foreign: [String: String] = [
        ...
        "CGEvent.tapCreate": "Core Graphics' event tap, named where the documents say which process TCC asks about when it fails",
        "AXIsProcessTrusted": "the Accessibility framework's own answer, read in the child to prove the grant was inherited",
        "xctest": "Xcode's test runner — named precisely because it is *not* what the grant goes to",
