...
final class RuleStampTests: XCTestCase {
    ...
    /// The marks are keyed now, so a test asks its questions under the same key
    /// the engine beside it uses — `StampMarkTests` is where the key's own
    /// properties live.
    ...
    func testAFreshFileIsNotStamped() throws {
        ...
        XCTAssertFalse(stamp.isStamped(url.path, by: "rule-1"))
    }

    func testStampingThenReading() throws {
        let url = try file("a.pdf")
        stamp.stamp(url.path, by: "rule-1")
        XCTAssertTrue(stamp.isStamped(url.path, by: "rule-1"))
    }
    ...
    func testTheStampIsPerRule() throws {
        let url = try file("a.pdf")
        stamp.stamp(url.path, by: "rule-1")
        XCTAssertFalse(stamp.isStamped(url.path, by: "rule-2"))
    }
    ...
    func testTheStampSurvivesAMove() throws {
        ...
        XCTAssertTrue(stamp.isStamped(moved.path, by: "rule-1"))
    }
    ...
    func testStampsAccumulate() throws {
        let url = try file("a.pdf")
        stamp.stamp(url.path, by: "rule-1")
        stamp.stamp(url.path, by: "rule-2")
        XCTAssertTrue(stamp.isStamped(url.path, by: "rule-1"))
        XCTAssertTrue(stamp.isStamped(url.path, by: "rule-2"))
    }
    ...
    func testStampingSaysWhetherItWorked() throws {
        let url = try file("a.pdf")
        XCTAssertTrue(stamp.stamp(url.path, by: "rule-1"))
        XCTAssertFalse(stamp.stamp(directory.appendingPathComponent("gone.pdf").path,
                                       by: "rule-1"))
    }
    ...
    func testAMissingFileReadsAsUnstamped() {
        XCTAssertFalse(stamp.isStamped(directory.appendingPathComponent("gone").path,
                                           by: "rule-1"))
    }
}
