...
| `activeTintColor` | String | green | Цвет host-иконки пока активно (выбор из палитры) |

**Цвет иконки:** неактивна → **белое** кольцо (template). Активна → `activeTintColor`, выбор из
фиксированной палитры (white/red/orange/yellow/green/mint/cyan/blue/purple/pink), default green.

**Форма host-иконки:** v1 — только **кольцо** (фиксировано, без настройки). Расширение форм — позже.
...
### Меню-бар (host-level)
- Одна иконка Helm — **кольцо** (фиксировано в v1; формы расширим позже).
- Клик → общая панель Helm.
- **Тинт:** неактивна → белое template-кольцо. Активна → модуль публикует `statusAppearance` (цвет
  `activeTintColor` из палитры) → host красит кольцо. Контракт получает точку `statusAppearance` —
  модуль может просить тинт/бейдж у host-иконки (при многих модулях host выбирает по приоритету).
...
Решены: иконка неактивна = белое кольцо, активна = цвет из палитры (default green); форма v1 = только
кольцо (расширение позже).
