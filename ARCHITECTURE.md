...
`Scripts/package-app.sh:77` names something the manifest says exists rather than
...
`Scripts/package-app.sh:135` signs ad-hoc (`codesign --force --deep --sign -`), so the
...
`Scripts/package-app.sh:46` computes the build number as `git rev-list --count HEAD`,
...
substituting a number, and `Scripts/package-app.sh:117` writes it into the plist *of the
...
`Scripts/package-app.sh:77` builds `swift build -c release --product HelmApp`, assembles
...
copied in by `Scripts/package-app.sh:88` — rather than reading the icon back, because macOS
